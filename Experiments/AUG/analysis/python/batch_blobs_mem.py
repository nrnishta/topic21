...
for shot in shotList:
    ...
    for strokes in ('1', '2', '3', '4', '5', '6'):
        ...
        if not np.isnan(D['tmin'+strokes].values[0]):
            print('Evaluating Data for shot %5i strokes number %1i', shot, int(strokes))
            ...
            if np.isnan(D['Threshold_' + strokes].values[0]):
                # no interelm
                print('Evaluating without ELM for shot %5i strokes number %1i', shot, int(strokes))
                ...
                print('Evaluating with ELM for shot %5i strokes number %1i', shot, int(strokes))
            ...
            print('No strokes %1i for shot %5i', int(strokes), shot)
