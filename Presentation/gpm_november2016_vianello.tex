...
\begin{frame}{Present status}
  ...
  \begin{enumerate}[<+(1) | invisible@-+>]
    ...
    \item The observations exhibit a dependence on I$_p$ (increasing
      the current at constant B$_{\phi}$ shoulder tends to disappear)
    \item The H-Mode behavior is still a work in progress and exhibit
      a dependence on the level of \alert{fueling}
    \item L-Mode and H-Mode behavior are not yet reconciled and
      different devices show somehow contradictory behavior
    \end{enumerate}
\end{frame}

\begin{frame}{L-Mode studies}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
      \item<1-> AUG and JET \parencite{Carralero:2015gu} suggest that $\Lambda_{div}$ dominates this
        ...
        \item<3> This determines a change of the velocity-size scaling
    ...
    \only<3>{
      \begin{tikzonimage}[width=\textwidth]{../pdfbox/KoM15Nov/CarraleroPRL15b}
          \draw [ultra thick, ta3chameleon, thick, rotate=1] (0.2,0.5) ellipse (0.05 and 0.4);
          \draw [thick, blue, ultra thick, rotate=18] (0.65,0.05) ellipse (0.35 and 0.1);
      \end{tikzonimage}
    }
    \end{column}
  \end{columns}
