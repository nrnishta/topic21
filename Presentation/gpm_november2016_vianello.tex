  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      controls shoulder formation
        \item<2-> ... which is associated with a change of the velocity-size
          scaling from
            \textcolor{ta3chameleon}{sheath-limited}
        to \textcolor{blue}{inertial regime}.
        \item<3-> \textcolor{red}{Tested by changing n$_e$
...
\begin{frame}{$n_{\text{proposed shots}} \gg n_{\text{allocated shots}}$}
	\begin{itemize}
		...
		\item Preliminary shot allocation. AUG: 14. MAST: 13. TCV: 23 !!!!!!!1SHOULD WE ADD PRELIMINARY DATES FOR THE EXPERIMENTS??
...
\begin{itemize}
	...
	\end{enumerate}
	\item Fueling in H-mode is problematic due to transport barrier? NBI, pellet fueling?
	\item H-mode density limit\parencite{Bernert:2015ppcf} must be dealt with
\end{itemize}
...
\begin{frame}{Proposed shot plan - TCV}
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\item Move plasma vertically. Disentangle $q_{95}$ and $L_{\|}$			
		\end{enumerate}
...
\begin{frame}{Preliminary shot plan  - MAST-U}
	\begin{itemize}
		...
		\begin{enumerate}		
			...
			\item Same $I_p$ and $q_{95}$ scans as on AUG. Availability of multi pin probe? 
			\item Try varying fueling location
			\item RFEA measurements during shoulder formation 
		\end{enumerate}
		\item H-mode (7 shots)
		\begin{enumerate}
			\item These experiments require the existence of H-mode reference shot. Perhaps not available in 2017
			\item Similar scenario development as on other machines
		\end{enumerate}
...
\begin{itemize}
	\item Detailed ion temp measurements 
...
\begin{frame}{Future and Miscellaneous}
	\begin{itemize}
		...
		\item Shortly after this meeting we will organize a follow-up meeting to 						reconcile ideas from this discussion (date not fixed yet)
		\item All information will be gathered on the wiki
		\item Filament database		
	\end{itemize}
\end{frame}

\begin{frame}{Discussion agenda}
	\begin{enumerate}
		\item Priorities. Do we agree? 
		\item Common measurement techniques. E.g. $I_{sat}$ instead if $\phi_f$ for filament velocities?
		\item Publications 
 	\end{enumerate}
\end{frame}
