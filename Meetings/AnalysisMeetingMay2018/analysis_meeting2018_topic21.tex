...
\usetheme[conference=Topic21-Analysis-Meeting,venue=Remote, date=17/05/2018, titleprogressbar, logo=RFX-logo]{Eurof}
...
\title{Topic 21 Experiment and analysis}
\date{17 May 2018}
...
\only<3>{
  ...
  \begin{itemize}
    ...
    \item $\tau_b$ estimated from FWHM of Conditional Avearage
      (\alert{different from Carralero}) including asymmetric part
      (\alert{different from Tsui})
  \end{itemize}
}
\only<4>{
  \begin{columns}
    \begin{column}{0.5\textwidth}
  \includegraphics[width=\textwidth]{../../Experiments/Comparison/pdfbox/ExampleCorrelation}
    \end{column}
    \begin{column}{0.5\textwidth}
  \begin{itemize}
    \item Correlation on TCV computed using a gaussian Fit of the
      central part (better resolution)
    \item on AUG the correlation is in general asymmetric. Maximum
      time lag estimated from the zeros of the imaginary part of the
      hilbert transform of the correlation function
  \end{itemize}
    \end{column}
  \end{columns}
}
