...
\begin{frame}{Background}
  \only<1>{
\begin{itemize}
\item Role of turbulence transport in the SOL density profile evolution is a well
  know
  feature
  \parencite{LaBombard:2001ks,Rudakov:2005ic,Garcia:2007hh,Carralero:2014gs}. \textcolor{red}{By
    increasing
  density, even without reaching detachment,  SOL density profile tends to flatten}
\end{itemize}}
\only<2-4>{
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item AUG and JET (HT) \parencite{Carralero:2015gu} suggest that $\Lambda_{div}=\frac{L_{\parallel}/c_s}{1/\nu_{ei}}\frac{\Omega_i}{\Omega_e}$ dominates this
        process through a transition from
  ...
  \begin{columns}[c]
    \begin{column}{0.4\textwidth}
      ...
      \item<6-7> On TCV \parencite{Vianello:2017ku} $\Lambda_{div}$
        ...
        in the far SOL
      \item<7|only@7> Additional mechanisms suggested among which
        \alert{effect of divertor neutrals (\emph{clogging mechanism})} 
    \end{itemize}
  \end{column}
...
\Fontvi
  \begin{itemize}
    ...
    \item \textbf{TCV:} A single j$_{sat}$ measurement available,
      which implies different velocity estimate. $v_r$ from
      $\mathbf{E}\times\mathbf{B}$ evaluation from floating potentials
      $v_z$ from 2D cross-correlation analysis ({\small \textit{see
          Tsui,  PoP submitted}})
  \end{itemize}
}
\only<3>{
  ...
  \begin{itemize}
    \item Blob-size is $\delta_b = \frac{\tau_b*v_{\perp}}{2}$
    \item $\tau_b$ estimated from FWHM of Conditional Average, 
      binormal blob velocity estimated 
  \end{itemize}
}
\end{frame}
    ...
    \begin{column}{0.3\textwidth}
      \begin{itemize}
        ...
        \only<9-10>{\item Amplitude evolve faster in density at lower
          ...
          the transition to highly-recycling regime as in 
          JET HT \parencite{Wynn:2018gp} \onslide<10>{\alert{but
              evolution is equivalent if considered \textit{vs} greenwald
            fraction }}
...
\begin{frame}{Neutral behavior investigation on AUG}
  \only<1>{
    \begin{columns}
      \begin{column}{0.5\textwidth}
        \begin{itemize}
  ...
  \only<2>{
    \begin{center}
    \begin{tikzonimage}[width=10cm]{../../Experiments/AUG/analysis/pdfbox/Dalpha/Agostini_Fig2.pdf}
        \node at (0.05, -0.02) {\textbf{D$_{\alpha}$}};
        \node at (0.6, -0.02) {\textbf{D$_{\gamma}$}};
      \end{tikzonimage}
    \end{center}
  ...
  \only<3>{
    \begin{center}
    \begin{tikzonimage}[width=10cm]{../../Experiments/AUG/analysis/pdfbox/Dalpha/Agostini_Fig3.pdf}
        \node at (0.05, -0.02) {\textbf{D$_{\alpha}$}};
        \node at (0.6, -0.02) {\textbf{D$_{\gamma}$}};
      \end{tikzonimage}
    \end{center}
    
Radiation starts moving towards the X-point (different patterns D$_{\alpha}$, D$_{\gamma}$)
  }
  \only<4>{
    \begin{center}
    \begin{tikzonimage}[width=10cm]{../../Experiments/AUG/analysis/pdfbox/Dalpha/Agostini_Fig4.pdf}
        \node at (0.05, -0.02) {\textbf{D$_{\alpha}$}};
        \node at (0.6, -0.02) {\textbf{D$_{\gamma}$}};
      \end{tikzonimage}
    \end{center}
...
\only<2>{
  \centering{\includegraphics[width=10cm]{../../Experiments/AUG/analysis/pdfbox/Carralero_powerbalance}}
  \begin{itemize}
...
\begin{frame}{H-Mode analysis on AUG}
  ...
  \centering{\includegraphics<4>[height=0.75\textheight]{../../Experiments/AUG/analysis/pdfbox/UpstreamDivertorProfiles34102_34276_34281}}
  \centering{\includegraphics<5>[width=\textwidth]{../../Experiments/Comparison/pdfbox/EfoldSizeShots34276_34278}}
  \centering{\includegraphics<6>[width=\textwidth]{../../Experiments/Comparison/pdfbox/EfoldSizeShots34276_34278_34281}}
  \end{column}
